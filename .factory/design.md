...
- `site/public/registration-press.webp`: original hero illustration generated for this product using the factory image generator (`/opt/fleet/lib/gen-image.sh`, factory-image model), then resized/encoded locally as WebP. Prompt: “Wide editorial risograph collage for a developer CLI landing page; three translucent ink plates representing Node, Docker, and Python environment parsers overlap around a central registration target; torn warm paper, halftone dots, misregistered cyan, vermilion and violet inks, black terminal fragments with abstract key/value lines but no readable text; tactile screen-print imperfections; strong composition with breathing room; no logos, no brand marks, no gradients, no photorealism, no watermark.” The image is used as explanatory atmosphere and receives descriptive alt text.
